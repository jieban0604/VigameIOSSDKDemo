...
//#include "pthread.h"
#include <mutex>//modified by Tyson
...
#define Rename rename
	...
	// fix: modify PACKAGE_FILE_SIGN to VIZP
	const u32 PACKAGE_FILE_SIGN = 'VIZP';
	...
	public:
		...
		Package(FILE* file, bool readonly, bool readFilename);
		~Package();
	...
	private:
		...
		mutable std::mutex mutex_;//modified by Tyson
...
// 		pthread_mutex_t* mutex_;
		//modified by Tyson
		Lock(std::mutex* mutex) : mutex_(mutex){ mutex_->lock(); }
		~Lock() { mutex_->unlock(); }
		std::mutex* mutex_;
	};
