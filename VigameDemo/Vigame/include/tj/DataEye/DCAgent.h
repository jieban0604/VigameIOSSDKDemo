...
public:
	...
	*************************************************/
    ...
    /************************************************
     * Description: SDK上报频率，可以手动设置SDK的上报周期，
                    单位为秒，但该值不能小于30秒，否则无效。
     * time       : 上报间隔
     ************************************************/
    static void setUploadInterval(unsigned int time);
...
public:
	...
	*************************************************/
     ...
     * levelId       : 关卡ID
     *************************************************/
    static void paymentSuccessInLevel(const char* orderId, const char* iapId, double currencyAmount, const char* currencyType, const char* paymentType, const char* levelId);
};
...
{
	static std::function<void(void)> callback_updateSucc;
public:
	static void onUpdateSuccess();
public:
	/************************************************* 
	* Description: 在线参数更新接口
	*************************************************/
    static void update(const std::function<void(void)>& onFinish);
...
#define  LOGDC(...)  vigame::log2("DCLog", __VA_ARGS__)
